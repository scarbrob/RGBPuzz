...
export default function DailyChallengePage() {
  ...
  const resetPuzzle = () => {
    ...
    sessionStorage.removeItem(`rgbpuzz-local-${today}`);
  ...
  const saveDailyStats = (won: boolean, attemptCount: number, solveTime: number) => {
    const today = getLocalDate();
    const raw = localStorage.getItem('rgbpuzz-daily-stats');
    ...
    if (won) {
      stats.totalWins++;
      if (stats.fastestTime === null || solveTime < stats.fastestTime) {
    ...
    localStorage.setItem('rgbpuzz-daily-stats', JSON.stringify(stats));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.correct) {
        ...
        if (!statsUpdated) {
          saveDailyStats(true, attempts + 1, getCurrentSolveTime());
        ...
        if (!statsUpdated) {
          saveDailyStats(false, attempts + 1, getCurrentSolveTime());
          setStatsUpdated(true);
        }
      } else {
