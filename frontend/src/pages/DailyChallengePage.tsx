...
export default function DailyChallengePage() {
  ...
  const { saveStats } = useDailyStats('rgbpuzz-daily-stats')
  const [challengeDate, setChallengeDate] = useState(today)

  const game = useGameState({
    mode: 'daily',
    sessionKey: `rgbpuzz-local-${today}`,
    maxAttempts: DAILY_CHALLENGE_CONFIG.maxAttempts,
  ...
  useEffect(() => {
    const fetchChallenge = async () => {
      ...
      try {
        ...
        game.setError('⚠️ Could not load today\'s puzzle. Please check your connection and refresh.')
      }
      game.setLoaded()
    }
    fetchChallenge()
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in px-4">
      ...
      <div className="text-center mb-6 sm:mb-8 md:mb-10 mt-4 sm:mt-6 md:mt-8">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-violet-600 via-purple-500 to-fuchsia-500 dark:from-violet-400 dark:via-purple-400 dark:to-fuchsia-400 bg-clip-text text-transparent pb-2">
          Daily Challenge
        </h1>
      ...
      <div className="mb-6 sm:mb-8 animate-slide-in">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="text-base sm:text-lg">
            ...
            <span className="font-bold ml-2 text-xl sm:text-2xl text-light-accent dark:text-dark-accent">{game.attempts}</span>
          ...
          <div className="text-light-text-secondary dark:text-dark-text-secondary text-xs sm:text-sm font-medium">
            Order by <span className="text-light-accent dark:text-dark-accent">lowest</span> to <span className="text-light-accent dark:text-dark-accent">highest</span> RGB value
          </div>
        </div>

        <SortingGuide variant="rgb" />
        <TutorialHint />
        ...
        {game.isLoading ? (
          ...
          <GameStage variant="rgb">
            ...
            <div className="mt-6 sm:mt-8 flex justify-center">
              <button 
                className={`w-full max-w-md text-base sm:text-lg py-3 sm:py-4 ${
                  game.gameState === 'won' 
                    ? 'game-button !bg-none !bg-emerald-500 dark:!bg-emerald-500 !text-white !shadow-[0_2px_12px_rgba(16,185,129,0.4)] cursor-default' 
                    : 'game-button'
                }`}
                onClick={game.handleSubmit} 
      ...
      {(game.gameState === 'won' || game.gameState === 'lost') && (
        <ShareResult title="RGBPuzz" attempts={game.attempts} maxAttempts={game.maxAttempts} attemptHistory={game.attemptHistory} />
      )}
    </div>
  )
}
