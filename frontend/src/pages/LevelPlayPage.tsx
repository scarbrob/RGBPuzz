...
import { LEVELS_PER_DIFFICULTY, DIFFICULTY_CONFIG } from '../../../shared/src/constants'
...
export default function LevelPlayPage() {
  ...
  const game = useGameState({
    mode: 'level',
    sessionKey: `level-local-${difficulty}-${level}`,
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in px-4">
      ...
      <div className="mb-4 sm:mb-6">
        <button onClick={() => navigate('/levels')} className="game-button-secondary text-sm mb-3 sm:mb-4">← Back to Levels</button>
        <div className="text-center">
          ...
          <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">Attempts: {game.attempts}/{game.maxAttempts}</p>
        </div>
      </div>
            ...
            {game.gameState === 'won' && (
              <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-center">
                {parseInt(level || '0') < LEVELS_PER_DIFFICULTY && (
                  <button onClick={() => navigate(`/level/${difficulty}/${parseInt(level!) + 1}`)} className="game-button text-sm sm:text-base">Next Level →</button>
                )}
                <button onClick={() => navigate('/levels')} className="game-button-secondary text-sm sm:text-base">Level Select</button>
              </div>
            )}
            {game.gameState === 'lost' && (
              <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-center">
